...
function drawShape() {
    ...
    if(bonus1 !== undefined &&
        bonus1.x > paddle.x && bonus1.x < paddle.x + paddle.width && 
        bonus1.y + 40 > paddle.y && bonus1.y < paddle.y + paddle.height
    ){
        bouns = false;
        bonus1 = undefined;
        lives++;
    ...
    }else if(bonus1 !== undefined &&
        bonus1.x > paddle.x && bonus1.x < paddle.x + paddle.width && 
        bonus1.y > paddle.y && bonus1.y < paddle.y + paddle.height
        ){
        bouns = false;
        bonus1 = undefined;
    }
        ...
        for (let j = 0; j < blockDimn[i].length; j++) {
            ...
            if (
                ...
                if (blockDimn[i][j].health === 2) {
                    ...
                    Break_sound.play();
                    //Bouns
                    if(score_value != 0 && score_value % 2 == 0 && bouns === false){
                        bonus1 = new life(0, 0);
                        bonus1.x = blockDimn[i][j].x;
                        bonus1.y = blockDimn[i][j].y
                        bouns = true;
                    }
